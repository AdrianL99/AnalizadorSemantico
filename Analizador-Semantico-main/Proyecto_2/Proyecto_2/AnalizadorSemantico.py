...
class AnalizadorSemantico: #Aqui creamos la clase para el analizador semantico :,}
    def __init__(self):
        self.tabla = TablaHash.TablaHash()
        self.tokens = ['void','int','float','string']
        

    def buscar(self,num):#se refiere al objeto instanciado de esa clase sobre el cual se está invocando dicho método
    ...
    def crear_tabla(self): #aqui es donde se crea la tambla
        contador = 0
        cuerpo = ""
        booleana = bool(0)
        contador2 = 0

        with open("codigo.txt", "r") as f: #en esta parte del codigo es donde lee y abre el archivo a ejecutar
            for linea in f:
    ...
    def _cualTipoes(self,tipo):# se asignan los metodos operativos que se ejecutan
        if tipo == "int":
            return int
        if tipo == "string":
            return str
    ...
    def _imprimirArchivo(self):  #imprime lo que el archivo hace
        contador = 1
        with open("codigo.txt", "r") as f:
            for linea in f:
